...
class ApiClientManager:
    ...
    def set_use_zhipuai(cls, use_zhipuai=True):
        ...
        logger.info(
            f"API client manager: ZhipuAI mode switched from {previous} to {USE_ZHIPUAI}")
    ...
    def _worker_loop(self, worker_id):
        ...
        while not self._stop_event.is_set() or not self._request_queue.empty():
            try:
                # Get request
                try:
                    priority, request_id, (request_func, args, kwargs,
                                           result_holder) = self._request_queue.get(timeout=0.1)
                except queue.Empty:
                    continue
                ...
                try:
                    ...
                    response = self._execute_request_internal(
                        request_func, *args, **kwargs)

                    # Validate response
                    if not self._validate_response(response):
                        raise ValueError(
                            f"API returned invalid response: {response}")
                    ...
                    if result_holder.get("retry_count", 0) < self.max_retries:
                        ...
                        result_holder["retry_count"] = result_holder.get(
                            "retry_count", 0) + 1
                        # Simple linear backoff
                        retry_delay = 0.5 * result_holder["retry_count"]
                        ...
                        logger.error(
                            f"Request failed, max retries reached: {str(e)}")
                ...
                logger.error(
                    f"Worker thread {worker_id} error processing request: {str(e)}")
    ...
    def _execute_request_internal(self, request_func, *args, **kwargs):
        ...
        try:
            ...
            self.error_count += 1
            logger.error(
                f"API request error, time: {time.time() - start_time:.2f}s, {str(e)}")
            raise
            ...
            if client_key not in self._clients:
                try:
                    if USE_ZHIPUAI:
                        ...
                        logger.info(
                            f"Initialized {'ZhipuAI' if USE_ZHIPUAI else 'OpenAI'} API client: {'' if USE_ZHIPUAI else base_url}")
    ...
    def execute_request(self, request_func, *args, priority=5, **kwargs):
        ...
        self._request_queue.put(
            (priority, request_id, (request_func, args, kwargs, result_holder)))
    ...
    def execute_request_batch(self, request_funcs_with_args):
        ...
        for func, args, kwargs in request_funcs_with_args:
            future = self.executor.submit(
                self.execute_request, func, *args, **kwargs)
